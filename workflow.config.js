...
new Workflow('to-self', {
  description: '提交到远程',
  steps: [
    { name: '获取提交信息', use: Tasks.inquireCommitDetails() },
    { name: '提交', use: Tasks.gitCommit(message => [{ message }]) },
    { name: '推送', use: Tasks.gitPush() },
  ],
});
...
new Workflow('build-and-link', {
  description: '构建并本地测试',
  steps: [
    { name: '构建', use: Tasks.runCmd({ cmd: 'npm run build' }) },
    { name: '本地软链', use: Tasks.runCmd({ cmd: 'npm link' }) },
  ],
});
